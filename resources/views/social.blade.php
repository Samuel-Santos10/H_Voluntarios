

@extends('layout')

@section('contenidos')



        <div>
        <img src="img/Si.jpg" alt="">
            <ul> <h5>INDICE</h5> 

                <li>INTRODUCCION</li>
                <li>¿Qué debes saber?</li>
                <li>Áreas sociales en las que puedes ayudar</li>
                <li>¿Errores de los que puedes ser víctima en tus inicios?</li>
                <li>Ventajas</li>
                <li>Recomendaciones</li>
            </ul>

        </div>


        <div class="row py-4">
        <img src="img/soc2.jpg" alt="">
        <h6>INTRODUCCION</h6> <br>

        <p>
        El programa de formación e inducción está dirigido a todas aquellas personas que quieran contribuir a la protección social de una forma altruista y desinteresada.
        <br>

        Creemos que cada ciudadano tiene la oportunidad de participar en la sociedad, sin importar su edad, género, preferencia sexual, afiliación política o creencia religiosa. Invitamos a todos a unirse a nuestra causa solidaria. <br>

        Nuestro enfoque se basa en la sencilla idea de que podemos "cambiar el mundo, una persona a la vez". Con esta visión en mente, hemos adaptado el programa de voluntariado social, para hacer que esto suceda.
        <br>

        Esta experiencia te permite conocer nueva gente las cuales tienen los mismos intereses que tú, tambien te permite fortalecer lazos con la comunidad y ampliar tu red de apoyo en la cual llegaras a muchas personas que necesitan de ti.

        </p>
        </div>


        <div class="row py-4">
        <h6>¿Qué debes saber?</h6> <br>

        <img src="img/soc3.jpg" alt="">

        <p>
        Como primer requisito debes de conocimientos básicos los cuales te califican para poder formar parte de esta arrea social. <br>

        En este contenido te daremos a conocer las diferentes áreas en las desenvolver y en la cual puedes ofrecer tu ayuda, en la cual podrás solucionar diferentes problemas que las personas de diferentes edades, estas pueden ser de cuidar personas mayores hasta realizar un servicio a una determinada comunidad. <br>

        Al formar parte recibirás una gran responsabilidad, pero no te preocupes, nosotros te guiaremos en base a el contenido que te presentaremos con el fin de que te puedas adaptar, por ello para facilitar aún más tu estadía te brindaremos diferentes ilustraciones para qué tengas una experiencia que al final del día te deje satisfecho con las acciones que realizas.  

        </p>
        </div>


        <div class="row py-4">
        <h5>ÁREAS SOCIALES EN LAS QUE PUEDES AYUDAR</h5> <br>
        </div>


   <!--Card-->
        <div class="row row-cols-1 row-cols-md-2 g-5">
  <div class="col">
    <div class="card">
      <img src="img/soc5.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Casa hogar para adultos mayores</h5>
        <p class="card-text">Las casas hogares son los lugares donde mayor cuidado se necesitan, ya que en esta área debes de esta siempre pendiente de los residentes de estos hogares, tambien debes de ser paciente ya que los adultos mayores presentan cambios de humor constante, así como tambien debes de estar siempre pendiente de que estos tomen sus medicinas, coman sus alimentos diarios, asearlos y entras actividades, con los niños es algo similar ya que tambien debes de estar pendiente de ellos, jugar con ellos, así como tambien debes de orientarlos de la manera adecuada, es decir, que debes de enseñarles valores, para que estos al crecer siempre tengas una buena base de cómo tratar a los mayores, así como tambien debes de enseñarles a escribir y leer para que estos tengas una base cuando decidan salir al mundo.</p>
      </div>
    </div>
  </div>
  <div class="col">
    <div class="card">
      <img src="img/soc4.jpeg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Aseo</h5>
        <p class="card-text">Si eres un amante de la limpieza esta es tu área,  puedes contribuir no solo a diferentes comunidades las cuales con tu ayuda estas se convertirán en comunidades sanas, tambien estas ayudando a que las personas puedan padecer de enfermedades, ya que se sabe que en lugares donde se desecha mucha basura y a medida esta se va almacenando provoca ciertos hedores que para aquellas personas con problemas respiratorios puede causarles grandes daños en su salud, así como tambien estas ayudando a un medio ambiente sano y próspero en el cual tu eres el pionero de este gran cambio.</p>
      </div>
    </div>
  </div>
  <div class="col">
    <div class="card">
      <img src="img/discapacidad.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Discapacidad</h5>
        <p class="card-text">La discapacidad es una de las áreas mas olvidadas de nuestra sociedad, en esa área de voluntariado debes de tener en cuenta que en primer lugar al formar parte de esta área le estas mejorando la calidad e vida y la autonomía de las personas con diversidad de funcional o discapacidad, en esta área debes de tener en cuenta que debes de entablas una relación muy cercana a la persona, es decir, tienes que acompañarla en su vida diaria, tambien debes de apoyarla en actividades de integración de la sociedad, en sus actividades de inserción laboral y en el apoyo familiar, por ello al ser de esta área debes de tener en cuenta todas estas  condiciones que te permitiran ser de esta area.</p>
      </div>
    </div>
  </div>
  <div class="col">
    <div class="card">
      <img src="img/soc7.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Adicciones</h5>
        <p class="card-text">Tienes que tener en cuenta que hay diferentes tipos de adiciones, entre estas tenemos la adicción de sustancias de fármacos, bebidas alcohólicas, adicción a redes sociales entre otras. <br>
      
        Si te haz decidido de formar parte de esta área social, entonces tu labor voluntaria se basa en ayudar tanto en la prevención y recuperación de las adicciones, para ello debes de tener en cuenta que tener cierto grado de conocimiento o razonamiento mental, puesto que las tareas que realizaras para que las personas adictivas puedan recuperarse son, rehabilitaciones, en la cual puedes dar charlas, en las cuales estas tienen como fin darles apoyo psicológico a estas personas, así como acompañamiento durante el proceso en el que este se encuentre en su rehabilitación.   
    </p>
      </div>
    </div>
  </div>
  <div class="col">
    <div class="card">
      <img src="img/familia.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Infancia, juventud y familia</h5>
        <p class="card-text">Esta área tienes que tener en cuenta que las acciones que tienes como labor son llevar tanto a niños como adolescentes y familias son, motivarlas a participar en proyectos de ocio y de tiempo libre, tambien en proyectos educativos, debes de divulgarles los derechos que estos poseen, debes de integrarlos a programas o eventos que se realizan en la comunidad con el fin de romper esa barrera que las mismas personas se ponen hacia con las demás personas. </p>
      </div>
    </div>
  </div>
</div>
<!--Card-->

<div class="row py-4">
        <h5>¿Errores de los que puedes ser victima en tu inicio?</h5> <br>

        <img src="img/err.png" alt="">
</div>

<div class="row py-4">
        <h5>Ventajas</h5> <br>

        <img src="img/ventajas.jpg" alt="">
</div>


        <div class="row py-4">
                <h5>RECOMENDACIONES</h5> <br>

                        <!--Card-->
                        <div class="row row-cols-1 row-cols-md-2 g-4">
                <div class="col">
                    <div class="card">
                    <img src="img/soc9.jpg" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Disponer de tiempo libre</h5>
                        <p class="card-text">Las diversas actividades en las que formaras parte conllevan gran parte de tiempo libre, por lo que debes de tener tiempo para que puedas realizar tus acciones beneficas.</p>
                    </div>
                    </div>
                </div>
                <div class="col">
                    <div class="card">
                    <img src="img/iniciativa.jpg" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Iniciativa</h5>
                        <p class="card-text">Debes de ser una persona con mucha dispocision he iniciativa ya que hay areas en las que debes de ser una persona con una mentalidad dinamica y emprenderdora.</p>
                    </div>
                    </div>
                </div>
                <div class="col">
                    <div class="card">
                    <img src="img/Conocimiento.jpg" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Conocimientos</h5>
                        <p class="card-text">Debes ser una persona con conocimientos basicos de acuerdo al area en la que quieres formar parte.</p>
                    </div>
                    </div>
                </div>
                <div class="col">
                    <div class="card">
                    <img src="img/responsabilidad.jpg" class="card-img-top" alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Responsable</h5>
                        <p class="card-text">Ante todo debes ser conciente del compromiso que adquiriras al ser parte de un voluntariado, por lo cual debes de ser conciente del dasafio al que haz emprendido y tomarlo con la mayor responsabilidad.</p>
                    </div>
                    </div>
                </div>
                </div>
            <!--Card-->

        </div>

        <footer>
        
        <div class="col-12 align-self-center text-center">
    <h3>PAGINAS WEB DE VOLUNTARIADO SOCIAL</h3>

    <p>Te brindamos los diferentes enlaces como unico fin de que puedas adquirir mas conocimientos en esta<br>
     area del voluntariado social, el cual te ayudara mucho en tu formacion hacia el objetivo que deseas lograr,<br> 
     ya que sabemos que estas anseoso de comenzar a desempeñar tu labor de la mejor.</p>


        <p>
        <h5>¿Qué más debes saber de voluntariado social?</h5>
        <a href="https://www.juntadeandalucia.es/organismos/igualdadpoliticassocialesyconciliacion/areas/inclusion/voluntariado-social/paginas/introduccion-voluntariado.html"target="_blank">Ir al sitio</a>
        <br>
        <h5>Cultural en el área social</h5>
        <a href="https://freiwilligendienste-kultur-bildung.de/es/"target="_blank">Ir al sitio</a>
        <br>
        <h5>Discapacidad</h5>
        <a href="http://riberdis.cedd.net/handle/11181/2879"target="_blank">Ir al sitio</a>
        <br>
        <h5>Migración y refugio</h5>
        <a href="https://coordinadoraongd.org/nuestro-trabajo/migracion-y-refugio/"target="_blank">Ir al sitio</a>
        <br>
        <h5>Sin hogar</h5>
        <a href="https://www.solidarios.org.es/que-hacemos/personas-sin-hogar/"target="_blank">Ir al sitio</a>
        <br>
        <h5>Consejos para ser un buen voluntariado</h5>
        <a href="https://masoportunidades.org/top-10-consejos-voluntariado-voluntario-responsable/"target="_blank">Ir al sitio</a>
        <br>
        </p>
    </div>


        </footer>


@endsection